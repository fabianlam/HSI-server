...
def get_lan_ip():
    ...
    try:
        # Doesn't need to actually connect - just to pick interface
...
def fetch_hsi_futures():
    ...
    # Find table by content keywords
    table = None
    ...
    for row in table.find_all("tr"):
        ...
        if texts and "HANG SENG INDEX Futures" in texts[0]:
            try:
                current = float(texts[1].replace(",", ""))
                change = float(texts[2].replace(",", ""))
                change_pct = float(texts[3].rstrip("%").replace(",", ""))
                open_val = float(texts[6].replace(",", "")) if texts[6].strip() else None
                low = high = None
                if "-" in texts[7]:
                    parts = [p.strip().replace(",", "") for p in texts[7].split("-")]
                    if len(parts) == 2:
                        low, high = float(parts[0]), float(parts[1])
                last_close_approx = round(current - change, 2) if change is not None else None
                # Timestamp
